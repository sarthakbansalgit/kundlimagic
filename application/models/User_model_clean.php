...
class User_model extends CI_Model
{
    ...
    public function create_user($data)
    {
        ...
        $result = $this->mongodb_api->insert($this->collection, $data);
        return $result ? $data['_id'] : false;
    }

    public function login($email, $password)
    {
        $user = $this->mongodb_api->find_one($this->collection, ['email' => $email]);
    ...
    {
        $user = $this->mongodb_api->find_one($this->collection, ['_id' => $user_id]);
    ...
    {
        $data['updated_at'] = $this->mongodb_api->get_timestamp();
        return $this->mongodb_api->update_one($this->collection, ['_id' => $user_id], $data);
    }
    ...
    public function get_user_by_phone($phone)
    {
        ...
        $user = $this->mongodb_api->find_one($this->collection, ['whatsapp' => $phone]);
        if ($user) {
            $user['id'] = $user['_id'];
            return $user;
        }
        ...
        $user = $this->mongodb_api->find_one($this->collection, ['phone' => $phone]);
        if ($user) {
            $user['id'] = $user['_id'];
            return $user;
        }
